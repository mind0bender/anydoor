...
import { CalendarDays, Clock3, Mail, Phone, Sparkles, UserRound } from "lucide-react";
import {
...
export default function GuestDetailsClient({ booking }: GuestDetailsClientProps) {
  ...
  const canContinue = fullName.length > 1 && email.includes("@") && phone.replace(/\D/g, "").length >= 7;

  return (
    <section className="w-full bg-[#d5fff9] text-[#003531]">
      <div className="w-full max-w-[1200px] mx-auto px-4 sm:px-8 py-10 lg:py-12 grid lg:grid-cols-[1.15fr_0.85fr] gap-8">
        <div className="space-y-8">
          <header className="space-y-3">
            ...
            <p className="text-[#1e5a52] max-w-2xl">
              Add the guest information so the reservation stays connected to the service and schedule you already selected.
            </p>
          </header>

          <div className="rounded-3xl bg-[#b3efe5] p-5 sm:p-6 space-y-5">
            <div className="grid sm:grid-cols-2 gap-4">
              <StatCard icon={<Sparkles size={16} />} label="Service" value={booking.service} />
              <StatCard icon={<CalendarDays size={16} />} label="Date" value={booking.date} />
              <StatCard icon={<Clock3 size={16} />} label="Time" value={booking.time} />
              <StatCard icon={<UserRound size={16} />} label="Provider" value={booking.staff} />
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field
                ...
                onChange={(value) => setGuest((current) => ({ ...current, fullName: value }))}
              ...
              <Field
                ...
                type="email"
                value={guest.email}
                onChange={(value) => setGuest((current) => ({ ...current, email: value }))}
              ...
              <Field
                label="Notes"
                value={guest.notes}
                onChange={(value) => setGuest((current) => ({ ...current, notes: value }))}
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  router.push(`/bookings/payment?${bookingQuery}&${guestQuery}`);
                }}
        ...
        <aside className="lg:sticky lg:top-24 h-fit rounded-3xl bg-[#bdeee4] p-6 space-y-6 shadow-sm">
          <div>
            ...
            <p className="text-sm text-[#1e5a52]">Everything stays connected.</p>
          </div>

          <div className="relative aspect-[4/3] rounded-2xl overflow-hidden bg-white/80">
            <Image src={booking.image} alt={booking.service} fill className="h-full w-full object-cover" />
          </div>
          ...
          <div className="space-y-3 text-sm text-[#1e5a52]">
            ...
            <DetailRow label="Total" value={`$${(booking.price + booking.price * 0.085).toFixed(2)}`} strong />
          </div>
        </aside>
      </div>
    </section>
...
function DetailRow({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4 rounded-xl bg-white/70 px-4 py-3">
      <span>{label}</span>
      <span className={strong ? "font-bold text-[#003531]" : "font-semibold text-[#003531]"}>{value}</span>
    </div>
  );
}
