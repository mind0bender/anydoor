...
const priceOptions: PriceOption[] = [
  { value: "₹", label: "Under ₹900", description: "Everyday picks", min: 0, max: 900 },
  { value: "₹₹", label: "₹900 - ₹1,400", description: "Comfort", min: 900, max: 1400 },
  { value: "₹₹₹", label: "₹1,400 - ₹2,400", description: "Premium", min: 1400, max: 2400 },
...
export default function DiscoverClient({ category, initialView }: Props) {
  ...
  const filteredListings = useMemo(() => {
    return category.listings.filter((listing) => {
      const inPrice = price
        ? price.max
          ? listing.startingFrom >= price.min && listing.startingFrom <= price.max
  ...
  return (
    <div className="flex flex-col lg:flex-row lg:items-start gap-8">
      <aside className="w-full lg:w-[320px] flex-shrink-0 bg-white/95 rounded-3xl p-6 shadow-sm border border-white/60 flex flex-col gap-6 lg:sticky lg:top-6 lg:h-fit">
        ...
        <div className="grid grid-cols-2 gap-3">
          <PillButton active={price === null} full onClick={() => setPrice(null)}>
            All prices
          </PillButton>
      ...
      <section className="flex-1 flex flex-col gap-4 min-w-0">
        <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white/80 rounded-2xl border border-white/60 shadow-sm p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col gap-1">
              <h1 className="text-2xl sm:text-3xl font-semibold">{category.title}</h1>
              <p className="text-sm text-[#1e5a52]">{category.tagline}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 text-sm text-[#1e5a52] flex-wrap">
            <span>Showing {filteredListings.length} of {category.total} sanctuaries</span>
        ...
        <div className="bg-white/80 rounded-2xl border border-white/60 shadow-sm p-4 flex flex-wrap items-center gap-3 justify-between">
          <div className="flex items-center gap-2 text-sm text-[#1e5a52]">
            <SlidersHorizontal size={16} />
            <span>Curated for balance — showing nearby, top rated, and great value.</span>
          </div>
...
const PillButton = ({ children, active = false, full = false, onClick }: PillButtonProps) => (
  <button
    onClick={onClick}
    className={cn(
  ...
  <div className="flex items-center gap-1 rounded-full border border-white/60 bg-white/80 shadow-sm p-1">
    <button
      ...
      className={cn(
        ...
        view === "grid" ? "bg-[#0b6b5d] text-white shadow" : "text-[#0b6b5d] hover:bg-white"
      )}
    >
      Grid
    </button>
    <button
      ...
      className={cn(
        "px-3 py-1.5 rounded-full font-semibold transition",
        view === "list" ? "bg-[#0b6b5d] text-white shadow" : "text-[#0b6b5d] hover:bg-white"
      )}
    >
      List
    </button>
    ...
    >
      <div className={cn(imageWrapperClasses, "bg-white")}> 
        <Image
          ...
          sizes={isList ? "(max-width: 768px) 50vw, 320px" : "(max-width: 768px) 100vw, 50vw"}
        />
        {!isList && (
...
const Tag = ({ children, tone = "light" }: { children: React.ReactNode; tone?: "light" | "dark" }) => (
  <span
    className={cn(
