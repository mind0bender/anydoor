...
export default function PaymentHistoryClient() {
  const [history, setHistory] =
    useState<PaymentHistoryEntry[]>(getPaymentHistory);
      ...
      <div className="mx-auto max-w-325 px-4 sm:px-8 py-12 space-y-8">
        <header className="space-y-3">
          ...
          <p className="text-[#1e5a52] text-lg max-w-2xl">
            A local booking ledger stored in your browser so you can review past
            payments even before a backend database is added.
          </p>
        </header>
        ...
        <div className="rounded-3xl bg-white/80 border border-white/70 shadow-sm p-4 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 text-sm text-[#1e5a52]">
            ...
            <input
              ...
              className="w-full sm:w-md bg-transparent outline-none"
            />
          </div>
          <div className="text-sm text-[#1e5a52] font-semibold">
        ...
        {filtered.length === 0 ? (
          <div className="rounded-3xl bg-white/80 border border-white/70 shadow-sm p-8 text-center space-y-4">
            ...
            <Link
              href="/bookings"
              className="inline-flex rounded-full bg-[#0b6b5d] text-white px-6 py-3 font-semibold hover:opacity-90 transition"
            >
              Start a booking
            </Link>
          </div>
          ...
          <div className="grid gap-5">
            {filtered.map((entry) => (
              <article
                key={entry.id}
                className="rounded-3xl bg-white/90 border border-white/70 shadow-sm overflow-hidden grid lg:grid-cols-[220px_1fr]"
              >
                ...
                <div className="p-5 sm:p-6 space-y-4">
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
                    <Meta
                      ...
                      label="Guest"
                      value={entry.guestName || entry.guestEmail || "Unknown"}
                    />
                    <Meta
                      icon={<Mail size={14} />}
                      label="Email"
                      value={entry.guestEmail || "Not set"}
                    />
                    <Meta
                  ...
                  <div className="flex flex-wrap gap-3 pt-2">
                    ...
                    <Link
                      href="/bookings"
                      className="rounded-full bg-white border border-[#0b6b5d]/20 text-[#0b6b5d] px-5 py-2.5 font-semibold hover:bg-[#f4fffb] transition"
                    >
                      Book again
                    </Link>
                  </div>
...
function CodeBlock({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl bg-[#003531] text-[#e5fff7] px-4 py-3 overflow-hidden">
      <p className="text-[10px] uppercase tracking-wide text-[#9fd8cc] font-semibold">
        {label}
      </p>
